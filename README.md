...
```bash
git clone https://github.com/filecoin-station/voyager-impact-evaluator.git
cd voyager-impact-evaluator
...
```bash
forge create --rpc-url http://127.0.0.1:8545 --mnemonic "test test test test test test test test test test test junk" src/Voyager.sol:Voyager --constructor-args 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266
```
...
```bash
forge create --rpc-url $RPC_URL --private-key <your_private_key> src/Voyager.sol:Voyager --constructor-args $ADMIN_ADDRESS
```

To deploy using a local mnemonic secret, run:

```bash
forge create --rpc-url $RPC_URL --mnemonic $MNEMONIC_PATH src/Voyager.sol:Voyager --constructor-args $ADMIN_ADDRESS
```
